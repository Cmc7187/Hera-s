...
{
    if (p->max == 0) return 0;
    int temp= (p->tail - p->head + p->max) % p->max;
    return temp;
}
...
int main()
{
    ...
    /*Queue p1{}, p2{}, p4{}, p5{}, p6{};
    initQueue(&p1, 10);//初始化测试
    Queue p3 {new int[20],20,3,3 };
    initQueue(&p2, p3);//深拷贝构造测试
    initQueue(&p4, Queue{ new int[20],20,4,4 });//移动构造测试
    assign(&p5, p3);//深拷贝赋值测试
    assign(&p1, p3);//原赋值的对象不为0的深拷贝构造
    assign(&p6, Queue{ new int[21],21,5,5 });//移动赋值
    assign(&p1, Queue{ new int[21],21,5,5 });
    //assign(&tp, *p);*/
    return 0;
}